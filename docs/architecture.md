...
`apps/spfx-assistant` hosts the UI, `packages/governance` masks input, `packages/retrievers`
accesses Graph, `packages/llm-providers` calls the model, and `packages/core` owns shared contracts.
...
4. The API exchanges the user token through OBO. Its client assertion is signed by a non-exportable
   Key Vault key.
5. Graph Search runs with the delegated Graph token, so SharePoint permission trimming happens at
   the source. The retriever downloads only in-scope `.docx` candidates and extracts relevant sections.
...
evaluation corpus has eight synthetic documents and is deliberately small. Phase 6 is the planned
place for hybrid Azure AI Search retrieval and a meaningful scale comparison.

## Related decisions

The detailed reasoning lives in [ADRs 001-010](adr/). The cross-tenant boundary is documented by
