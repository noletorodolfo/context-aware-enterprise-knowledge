...
| Subscriptions need reconciling now     | restart the ingestion app, or `POST /admin/functions/renew` with the host key                                     |
| Non-HTTP triggers stopped running      | `bash scripts/assert-identity-storage.sh <group> <app>`: a key-based `AzureWebJobsStorage` disables them silently |
